class Fora::ForumThreadNotificationJob < ApplicationJob
  include Fora::Engine.routes.url_helpers

  def perform(forum_thread)
    send_emails(forum_thread) if Fora.send_email_notifications
    send_webhook(forum_thread) if Fora.send_slack_notifications
  end

  def send_emails(forum_thread)
    forum_thread.notify_users.each do |user|
      Fora::UserMailer.new_thread(forum_thread, user).deliver_later
    end
  end
